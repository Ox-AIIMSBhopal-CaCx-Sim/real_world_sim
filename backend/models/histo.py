"""Callable histopathology discrete-event simulation."""
...
from utils.resource_availability import (
    ScheduledResource, TaskScheduledResource, Schedule, DisruptableResource,
    apply_disruptions, create_schedule_from_params,
    create_task_schedules_from_params, union_schedules,
)
...
def run_histo(
    ...
    *,
    ...
    """Run one isolated histopathology simulation and return its outputs."""
    if seed is not None:
        np.random.seed(seed)
    ...
    def _execute() -> dict:
        ...
        CervicalBiopsyPatient.all_cervical_biopsies.clear()
        NonCervicalBiopsyPatient.all_non_cervical_biopsies.clear()
        HistoSlide.all_slides.clear()
        run_simulation()
